...
- generic [ref=e1]:
  ...
  - main [ref=e29]:
    - insertion [ref=e33]:
      - generic [ref=e36]:
        ...
        - link "Technical Reference" [ref=e39] [cursor=pointer]
      ...
      - generic [ref=e111]:
        ...
        - insertion [ref=e114]:
          - generic [ref=e117]:
            ...
            - link "Automation testing examples" [ref=e120] [cursor=pointer]
            - link "Test automation tools" [ref=e125] [cursor=pointer]
            - link "Software testing courses" [ref=e130] [cursor=pointer]
        - paragraph [ref=e135]:
  ...
  - insertion [ref=e195]:
    - generic [ref=e198]:
      ...
      - link "Language Resources" [ref=e201] [cursor=pointer]
      - link "Test scenario examples" [ref=e206] [cursor=pointer]
      - link "Computer Security" [ref=e211] [cursor=pointer]
...
```ts
  ...
  20 |     );
  21 |     const message = await dashboardPage.getMessage();
  22 |     await expect(message).toBeVisible();
> 23 |     await expect(page.url).toBe(`${process.env.BASE_URL}/secure`);
     |                            ^ Error: expect(received).toBe(expected) // Object.is equality
  24 |   });
  25 | 
  26 |   test("Login with invalid credentials", async ({
  27 |     loginPage,page,
  28 |   }) => {
  29 |     await loginPage.login(
  30 |       process.env.TEST_USERNAME!,
  31 |       process.env.TEST_PASSWORD!,
  32 |     );
  33 |     const message = await loginPage.getMessage();
  34 |     await expect(message).toBeVisible();
  35 |     await expect(page.url).toBe(`${process.env.BASE_URL}/login`);
  36 |   });
  37 | });
  38 | 
```